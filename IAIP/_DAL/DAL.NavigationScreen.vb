            ...
            Select Case context
                ...
                Case NavWorkListContext.LateFce
                    query = "SELECT fci.STRAIRSNUMBER AS [AIRS #], fci.STRFACILITYNAME AS Facility, fci.STRFACILITYCITY AS City, fci.LASTFCE AS [Most recent FCE], fci.STRCLASS AS Class, fci.STRCMSMEMBER AS [CMS Class], fci.STROPERATIONALSTATUS AS [Operating Status]
                            ...
                            WHERE (fci.STRCMSMEMBER = 'A' AND fci.LASTFCE < DATEADD(yy, -" & MIN_FCE_SPAN_CLASS_A & ", SYSDATETIME()))
                            OR (fci.STRCMSMEMBER = 'S' AND fci.LASTFCE < DATEADD(yy, -" & MIN_FCE_SPAN_CLASS_SM & ", SYSDATETIME()))"
                ...
                Case NavWorkListContext.PermitApplications
                    query = "SELECT app.STRAPPLICATIONNUMBER AS [App #],
                            ...
                            CASE WHEN trk.DATPERMITISSUED IS NOT NULL THEN 'Permit Issued' WHEN trk.DATTODIRECTOR IS NOT NULL AND (trk.DATDRAFTISSUED IS NULL OR trk.DATDRAFTISSUED < trk.DATTODIRECTOR) THEN 'To Director''s Office' WHEN trk.DATTOBRANCHCHEIF IS NOT NULL AND trk.DATTODIRECTOR IS NULL AND (trk.DATDRAFTISSUED IS NULL OR trk.DATDRAFTISSUED < trk.DATTOBRANCHCHEIF) THEN 'To Branch Chief' WHEN trk.DATEPAENDS IS NOT NULL THEN 'EPA 45-day Review' WHEN trk.DATPNEXPIRES IS NOT NULL AND trk.DATPNEXPIRES < SYSDATETIME() THEN 'Public Notice Expired' WHEN trk.DATPNEXPIRES IS NOT NULL AND trk.DATPNEXPIRES >= SYSDATETIME() THEN 'Public Notice' WHEN trk.DATDRAFTISSUED IS NOT NULL AND trk.DATPNEXPIRES IS NULL THEN 'Draft Issued' WHEN trk.DATTOPMII IS NOT NULL THEN 'To Program Manager' WHEN trk.DATTOPMI IS NOT NULL THEN 'To Unit Manager' WHEN trk.DATREVIEWSUBMITTED IS NOT NULL AND (dat.STRSSCPUNIT <> '0' OR dat.STRISMPUNIT <> '0') THEN 'Internal Review' WHEN app.STRSTAFFRESPONSIBLE IS NULL OR app.STRSTAFFRESPONSIBLE = '0' THEN 'Received' ELSE 'At Staff' END AS [Application Status],
                            CASE WHEN trk.DATPERMITISSUED IS NOT NULL THEN trk.DATPERMITISSUED WHEN trk.DATTODIRECTOR IS NOT NULL AND (trk.DATDRAFTISSUED IS NULL OR trk.DATDRAFTISSUED < trk.DATTODIRECTOR) THEN trk.DATTODIRECTOR WHEN trk.DATTOBRANCHCHEIF IS NOT NULL AND trk.DATTODIRECTOR IS NULL AND (trk.DATDRAFTISSUED IS NULL OR trk.DATDRAFTISSUED < trk.DATTOBRANCHCHEIF) THEN trk.DATTOBRANCHCHEIF WHEN trk.DATEPAENDS IS NOT NULL THEN trk.DATEPAENDS WHEN trk.DATPNEXPIRES IS NOT NULL AND trk.DATPNEXPIRES < SYSDATETIME() THEN trk.DATPNEXPIRES WHEN trk.DATPNEXPIRES IS NOT NULL AND trk.DATPNEXPIRES >= SYSDATETIME() THEN trk.DATPNEXPIRES WHEN trk.DATDRAFTISSUED IS NOT NULL AND trk.DATPNEXPIRES IS NULL THEN trk.DATDRAFTISSUED WHEN trk.DATTOPMII IS NOT NULL THEN trk.DATTOPMII WHEN trk.DATTOPMI IS NOT NULL THEN trk.DATTOPMI WHEN trk.DATREVIEWSUBMITTED IS NOT NULL AND (dat.STRSSCPUNIT <> '0' OR dat.STRISMPUNIT <> '0') THEN trk.DATREVIEWSUBMITTED WHEN app.STRSTAFFRESPONSIBLE IS NULL OR app.STRSTAFFRESPONSIBLE = '0' THEN trk.DATRECEIVEDDATE ELSE trk.DATASSIGNEDTOENGINEER END AS [Status Date]
